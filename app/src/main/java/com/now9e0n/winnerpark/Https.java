...
/*
 * Https.class
 * Https request, response 를 관리하는 class 입니다.
*/
public class Https
{
	/*
	...
	*/
	public JSONObject postRequest(String url_string, HashMap<String, String> params) {
		...
		try {
			...
			// params 에 image 가 있으면 변수에 담아 request 를 다르게 보낸다
			String image = null;
	...
	public JSONObject request(String url_string, HashMap<String, String> params) {
		...
		try {
			...
			String response = null;
			String inputLine; // 서버로 부터 받은 response 를 받을 변수
			while ((inputLine = in.readLine()) != null) {
				response = inputLine;
			}

			if (response != null) {
				// response 가 object 냐 array 에 따라 parse 를 다르게한다.
				try {
					obj = (JSONObject) JSONValue.parse(response);
				} catch (Exception e) {
					try {
    ...
    */
	...
	public String getTimestamp() {
        ...
        return Long.toString(timestamp_long);
	}

	/*
     * Map 형식으로 Key 와 Value 를 셋팅한다.
